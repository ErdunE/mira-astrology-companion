...
If we had more time, there are several areas where we would extend and improve Mira. On the **safety and AI side**, we would implement more sophisticated guardrails around emotional support, such as stronger content filters, escalation suggestions when users seem to be in crisis, and more explicit disclaimers about the non-clinical nature of the app. We would also invest in better prompt engineering and model experimentation, possibly adding a mechanism to compare responses from different Bedrock models in a controlled way.

From a **feature perspective**, we would like to add richer visualizations and historical views. Examples include timeline views of past charts, comparisons across different life periods, and more interactive chart exploration. We would also consider adding user-configurable “intention settings” that let them choose different tones or focal areas (e.g., career, relationships, self-reflection) that influence both the horoscope data we emphasize and the prompts we send to Bedrock.

On the **engineering and operations side**, future work could include more comprehensive testing (integration and end-to-end tests that hit the deployed API), more advanced deployment strategies (such as blue-green or canary deployments for Lambda and frontend versions), and better cost monitoring dashboards. These enhancements would move Mira closer to a production-ready, continuously evolving cloud service.
